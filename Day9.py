...
#****STRING ARE IMMUTABLE So when we use methods like upper(), lower() the variable is not changed, we make a copy of that variable
...
print(str1.index("Dan")) #As we can see, this method is somewhat similar to the find() method. The major difference being that index() raises an exception if value is absent whereas find() does not.
...
print(str1.find("is")) #As we can see, this method is somewhat similar to the index() method. The major difference being that index() raises an exception if value is absent whereas find() does not.
